...
public class MixinRenderSectionManager implements INvidiumWorldRendererGetter {
    ...
    private void init(ClientWorld world, int renderDistance, CommandList commandList, CallbackInfo ci) {
        ...
        if (Nvidium.IS_ENABLED) {
            ...
            ((INvidiumWorldRendererSetter) regions).nVidium$setWorldRenderer(renderer);
        }
    }
    ...
    private boolean isSectionVisibleBfs(RenderSection section) {
        // The reason why this is done is that since the bfs search is async,
        // it could be updating the frame counter with the next frame
        // while some sections that aren't updated/ticked yet still have the old frame id
